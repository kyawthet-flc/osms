        ...
        <div class="form-group exiting-file-wrapper" style="clear: both;mb-3">
            ...
            @foreach($subProduct->subProductImages??[] as $k => $subProductImage)
               @if( $subProductImage->image )
                ...
                <a style="font-size: 15px;text-align:center;padding: 2px 10px 1px;border:1px solid #14aa75;border-radius: 3px;" href="data:image/png;base64,{!! $encrptedImg !!}" 
                class="text-success magnific-popup-img"><i class="mdi mdi-eye"></i></a>
                
                 <a style="font-size: 15px;text-align:center;padding: 2px 10px 1px;border:1px solid #ff3225;border-radius: 3px;" class="text-danger mt-1" delete-by-link="yes" 
                 confirmationText="Are you sure to delete?" remove-element=".sub-product-image-{{ $k }}" 
                 del-redirect-url="{{ url()->full() }}" 
                 href="{{ route('product.sub_product_image.delete', $subProductImage) }}">
