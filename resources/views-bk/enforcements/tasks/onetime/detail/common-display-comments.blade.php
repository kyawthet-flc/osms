@php 
   $application->load(['onetimeComments', 'onetimeComments.fromOfficer', 'onetimeComments.toOfficer', 'onetimeComments.attachments']);
@endphp

@foreach ($application->onetimeComments as $comment)
    ...
    @forelse ($comment->attachments as $attachment)          
           <a onclick="window.open('{{ route('tasks.onetime.show_document', $attachment) }}', '_blank', 'fullscreen=yes'); return false;" 
              href="{{ route('tasks.onetime.show_document', $attachment) }}">
