  ...
  <div class="row">
      ...
      <div class="col-md-4">
        ...
        @if( in_array(request('applicationStatus'), ['submitted', 'resubmitted']) )
            <div class="col-md-4">
                <label for="" class="col-md-12 pl-1 mb-0"><b>Registration Fee</b></label>
                <label for="reg_fee_all">
                    <input type="radio" name="registration_fee" id="reg_fee_all" value="reg_fee_all"
                    @if(request('registration_fee', 'reg_fee_all') =='reg_fee_all') checked="checked" @endif /> All
                </label>
                <label for="not_requested">
                    <input type="radio" name="registration_fee" id="not_requested" value="pending"
                    @if(request('registration_fee') =='pending') checked="checked" @endif /> Pending
                </label>
                <label for="unpaid">
                    <input type="radio" name="registration_fee" id="unpaid" value="unpaid"
                    @if(request('registration_fee') =='requested') checked="checked" @endif /> Unpaid
                </label>
                <label for="paid">
                    <input type="radio" name="registration_fee" id="paid" value="paid"
                    @if(request('registration_fee') =='paid') checked="checked" @endif /> Paid
                </label>
            </div>
      ...
      @endif
        ...
        <div class="col-md-12 text-right mt-5">
            ...
            <a href="{{ route('tasks.drc.index',['applicationType' => request('applicationType'), 'applicationStatus' => request('applicationStatus'), 'ipp' => request('ipp'), 'taskType' => request('taskType')]) }}" class="btn btn-danger no-alert">Clear</a>
        </div>


        @if( !in_array(request('applicationStatus'), ['incomplete', 'auto-cancelled', 'approved', 'rejected']) )
            @php
                $returnFromDgCount =  auth()->user()->isDirector()? auth()->user()->directorDrcDecisionCaseCount(request('applicationType'), request('applicationStatus')): 0;
            @endphp
            <div class="col-md-12">
                <label>
                    <input type="radio" onChange="this.form.submit();" name="taskType" checked="checked" value="myTasks" />
                    My Tasks {!! caseCounter( $drcTasksByAppStatus[request('applicationStatus')] - $returnFromDgCount) !!}
                </label>

                <label><input type="radio" onChange="this.form.submit();" name="taskType" @if(request('taskType') === 'allTasks') checked="checked" @endif value="allTasks" /> All {!! caseCounter($drcCommonCountArr[request('applicationStatus')]) !!}</label>
