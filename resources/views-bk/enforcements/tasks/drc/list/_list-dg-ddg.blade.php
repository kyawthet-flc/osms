    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr class="application-tr">
                ...
                <td>
                    <!-- // has view permission -->
                    @if( auth()->user()->hasPermission('drc', 'view') )
                       <a class="btn btn-warning btn-sm" href="{{ route('tasks.drc.show', ['drcApplication' => $list]) }}">View</a>
                    @endif

                    @if( in_array(request('applicationStatus'),['submitted', 'resubmitted']) )

                    @if( $isMyCase )
                        @if( auth()->user()->hasPermission('drc', 'to-approve') )
                            ...
                            href="{{ route('tasks.drc.decisional_action') }}">To Approve</a>
                        @endif

                        @if( auth()->user()->hasPermission('drc', 'to-reject') )
                            ...
                            redirect-url="{{ url()->full() }}"
                            style="margin-bottom: 4px;width: 80px;height: auto;padding: 5px;"
                            href="{{ route('tasks.drc.decisional_action') }}">To Reject</a>
                        @endif
                    @endif
                ...
                <tr>
                    <td colspan="11">
                        ...
                        <button type="button" disabled="disabled"
                          name="actionTypes"
                          value="to-reject-all"
                          url="{{ route('tasks.drc.decisional_action') }}"
                          redirect-url="{{ url()->full() }}"
