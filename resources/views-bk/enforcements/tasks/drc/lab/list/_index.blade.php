...
<x-utils.data-table class="table" :ths="$cols">
    ...
    @foreach ($lists as $k => $list)
        <tr>
            ...
            <td>
            <a class="btn btn-warning btn-sm view-lab-btn mt-1" href="{{ route('tasks.drc.show',['drcApplication' => $list]) }}">View Case</a>

            @if( in_array($list->drcActionRecord->lab_status,[
                'received', 'passed', 'failed'
            ]))
            <a class="btn btn-warning btn-sm view-lab-btn mt-1" href="{{ route('tasks.drc.lab_show',['drcApplication' => $list]) }}">View Lab</a>
            @else
