...
<div class="row mb-2">
    ...
    <div class="col-md-9">{{  $application->drcSampleInformations->sample_quantities }}</div>
</div>
<div class="row mb-2">
    <div class="col-md-3">{{ __('Sample Sent date') }}</div>
    <div class="col-md-9">{{  $application->drcSampleInformations->sample_send_date }}</div>
</div>
<div class="row mb-2">
    <div class="col-md-3">{{ __('Sample Receipt No') }}</div>
    <div class="col-md-9">{{  $application->drcSampleInformations->sample_receipt_no }}</div>
</div>
@php
            ...
            <tr>
                ...
                <td colspan="2">
                    <a onclick="window.open('{{ route('tasks.drc.show_document', $attachment) }}', '_blank', 'fullscreen=yes'); return false;"
                       href="{{ route('tasks.drc.show_document', $attachment) }}" class="btn btn-success">View</a></td>
                <td>
                    <input type="checkbox"
                           class="incompletes"
