@php 
   $application->load(['diacComments', 'diacComments.fromOfficer', 'diacComments.toOfficer', 'diacComments.attachments']);
@endphp

@foreach ($application->diacComments as $comment)
    ...
    @forelse ($comment->attachments as $attachment)          
           <a onclick="window.open('{{ route('tasks.diac.show_document', $attachment) }}', '_blank', 'fullscreen=yes'); return false;" 
              href="{{ route('tasks.diac.show_document', $attachment) }}">
