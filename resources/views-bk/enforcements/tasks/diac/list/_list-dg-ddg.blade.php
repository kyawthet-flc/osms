...
<div class="table-responsive">
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            @php
                $lastComment = $list->diacComments->where('from_officer_id', $directorId)->whereIn('to_officer_id', $dgOrddgIds)->sortDesc()->first();
                $directorComment = $lastComment? $lastComment->comment: '';
                $isMyCase = $list->diacActionRecord->assigned_officer_id  === auth()->user()->id;
            @endphp
            <tr class="application-tr">
                ...
                <td>
                    <!-- // has view permission -->
                    @if( auth()->user()->hasPermission('diac', 'view') )
                       <a class="btn btn-warning btn-sm" href="{{ route('tasks.diac.show', ['diacApplication' => $list]) }}">View</a>
                    @endif

                    @if( in_array(request('applicationStatus'),['submitted', 'resubmitted']) )

                    @if( $isMyCase )
                        @if( auth()->user()->hasPermission('diac', 'to-approve') )
                            ...
                            href="{{ route('tasks.diac.decisional_action') }}">To Approve</a>
                        @endif

                        @if( auth()->user()->hasPermission('diac', 'to-reject') )
                            ...
                            redirect-url="{{ url()->full() }}"
                            style="margin-bottom: 4px;width: 80px;height: auto;padding: 5px;"
                            href="{{ route('tasks.diac.decisional_action') }}">To Reject</a>
                        @endif
                    @endif
                ...
                <tr>
                    <td colspan="11">
                        ...
                        <button type="button" disabled="disabled"
                          name="actionTypes"
                          value="to-reject-all"
                          url="{{ route('tasks.diac.decisional_action') }}"
                          redirect-url="{{ url()->full() }}"
