<x-forms.form-tag :attrs="[
  ...
  <div class="row">
        ...
        <input type="hidden" name="hasExtension" value="{{ request('hasExtension') }}" />
        ...
        <div class="col-md-4"> 
            <x-forms.text-input :attrs="['name' => 'business_name', 'value' => request('business_name'), 'placeholder' => 'Business Name']" />
        </div>
        ...
        @if( in_array(request('applicationType'), ['renew']))
        <div class="col-md-4">
        <label>Is Old Renewal</label><br/>        
        <input type="radio" onChange="this.form.submit();" name="is_old_renewal" id="all" value=""
        checked="checked" /> <label for="all">All</label>
        <input type="radio" onChange="this.form.submit();" name="is_old_renewal" id="yes" value="yes"
        @if(request('is_old_renewal') =='yes') checked="checked" @endif /> <label for="yes">Yes</label>
        <input type="radio" onChange="this.form.submit();" name="is_old_renewal" id="no" value="no"
        @if(request('is_old_renewal') =='no') checked="checked" @endif /> <label for="no">No</label>
        </div>
        @endif
        <div class="col-md-12 text-right">
            <button type="submit" class="btn btn-success form-action-btn" name="action" value="search">Search</button>
            <a href="{{ route('tasks.diac.index',['applicationType' => request('applicationType'), 'applicationStatus' => request('applicationStatus'), 'hasExtension' => 'yes', 'ipp' => request('ipp'), 'taskType' => request('taskType')]) }}" class="btn btn-danger no-alert">Clear</a>
        </div>
        ...
        @if( !in_array(request('applicationStatus'), ['incomplete', 'auto-cancelled', 'approved', 'rejected']) )
            ...
            @php
              $returnFromDgCount =  auth()->user()->isDirector()? auth()->user()->directorDecisionCaseCount(request('applicationType'), request('applicationStatus')): 0;
            @endphp
            <div class="col-md-12">
                <label>
                    <input type="radio" onChange="this.form.submit();" name="taskType" checked="checked" value="myTasks" /> 
                    My Tasks {!! caseCounter( $diacTasksByAppStatus[request('applicationStatus')] - $returnFromDgCount ) !!}</label>
                <label>
                    <input type="radio" onChange="this.form.submit();" name="taskType" @if(request('taskType') === 'allTasks') checked="checked" @endif value="allTasks" />
                     All {!! caseCounter($diacCommonCountArr[request('applicationStatus')]) !!}
                </label>
