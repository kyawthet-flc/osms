...
<div class="table-responsive">
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            @php
                $lastComment = $list->drcComments->where('from_officer_id', $directorId)->whereIn('to_officer_id', $dgOrddgIds)->sortDesc()->first();
                $directorComment = $lastComment? $lastComment->comment: '';
                $isMyCase = $list->drcActionRecord->assigned_officer_id  === auth()->user()->id;
            @endphp
            <tr class="application-tr">
                ...
                <td>
                    <!-- // has view permission -->
                    @if( auth()->user()->hasPermission('drc-local', 'view') )
                       <a class="btn btn-warning btn-sm" href="{{ route('tasks.drc_local.show', ['drcApplication' => $list]) }}">View</a>
                    @endif

                    @if( in_array(request('applicationStatus'),['submitted', 'resubmitted']) )

                    @if( $isMyCase )
                        @if( auth()->user()->hasPermission('drc-local', 'to-approve') )
                            ...
                            href="{{ route('tasks.drc_local.decisional_action') }}">To Approve</a>
                        @endif

                        @if( auth()->user()->hasPermission('drc-local', 'to-reject') )
                            ...
                            redirect-url="{{ url()->full() }}"
                            style="margin-bottom: 4px;width: 80px;height: auto;padding: 5px;"
                            href="{{ route('tasks.drc_local.decisional_action') }}">To Reject</a>
                        @endif
                    @endif
                ...
                <tr>
                    <td colspan="11">
                        ...
                        <button type="button" disabled="disabled"
                          name="actionTypes"
                          value="to-reject-all"
                          url="{{ route('tasks.drc_local.decisional_action') }}"
                          redirect-url="{{ url()->full() }}"
