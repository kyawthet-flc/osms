...
use App\Model\Task\Drc\DrcIncomplete;
...
use DateTime;
use App\Templates\Drc\Certificate as DrcCertificate;
...
class ActionController extends Controller
{
    ...
    public function previewRejection(Request $request)
    {
    }
    ...
    {
        return (new DrcCertificate($drcApplication))->generate();
    }

    public function approve(Request $request, DrcApplication $drcApplication)
    {
        if($drcApplication->registration_fee != 'paid') {
            return $this->jsonResponse('error', 'This application has not been paid registration fee!', request('redirectUrl'));
        }
        ...
        DB::transaction(function () use(&$request, &$drcApplication){
            ...
            if($drcApplication->application_type == 'new'){
                ...
                $template = new DrcCertificate($drcApplication);
    ...
    public function requestRegFee(Request $request, DrcApplication $drcApplication) {
        if($drcApplication->registration_fee == 'paid') {
            return $this->jsonResponse('error', 'This application has been paid registration fee!', request('redirectUrl'));
        }
        if($drcApplication->registration_fee == 'requested') {
            return $this->jsonResponse('error', 'Already requested for registration fee for this application!', request('redirectUrl'));
        }
        DB::transaction(function() use (&$request,$drcApplication) {
            $drcApplication->update(['registration_fee' => 'requested']);

            $drcApplication->drcActionRecord->update([
                'request_reg_fee_at' => now(),
                'assigned_officer_id' => null,
                'org_assigned_officer_id' => auth()->user()->id
            ]);
        ...
        return $this->jsonResponse('success', "Successfully requested registration fee for " . $drcApplication->application_no, request('redirectUrl'));
    }
}
