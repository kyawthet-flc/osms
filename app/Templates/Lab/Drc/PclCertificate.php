...
class PclCertificate {
    ...
    {
        $this->template = File::get(storage_path('templates/lab/drc/pcl-certificate.html'));
        $configs = [
            'margin_left' => 11,
            'margin_header' => 13,
            'margin_right' => 11,
    ...
    {
        $labOverview = $this->drcApplication->labResult->labPcl;
        $redColorClass = ($labOverview->labPclTests()->where('conclusion','failed')->count() > 0)? 'red-text-calss': '';
 
        return array(
            'RED_COLOR_CLASS' => $redColorClass,

            'SUB_LAB_TITLE' => 'Pharmaceutical Chemistry Laboratory',
    ...
    {
        $redColorClass = (optional($this->drcApplication->labResult->labPcl)->labPclTests()->where('conclusion','failed')->count() > 0)? 'red-text-calss': '';
        $tpl = NULL;

        foreach (optional($this->drcApplication->labResult->labPcl)->labPclTests?? [] as $index => $pclTest) {
            $tpl .= '<tr>
                <td class="text-center sample-result-value" style="font-size: 14px;">' . ($index+1) . '.</td>
                <td class="text-center sample-result-value" style="font-size: 14px;">'. $pclTest['test_param'] .'</td>
                <td class="text-center sample-result-value" style="font-size: 14px;"><span class="'.$redColorClass.'">'. $pclTest['result'] .'</span></td>
                <td class="text-center sample-result-value" style="font-size: 14px;">'. $pclTest['specification'] .'</td>
                <td class="text-center sample-result-value" style="font-size: 14px;">'. $pclTest['ref_method'] .'</td>
                <td class="text-center sample-result-value" style="font-size: 14px;">'. $pclTest['conclusion'] .'</td>
            </tr>';
        }
    ...
    public function generate($string = null)
    {
        ...
        $stylesheet2 = file_get_contents(asset('css/lab-sections/pcl.css'), false, stream_context_create($arrContextOptions));
